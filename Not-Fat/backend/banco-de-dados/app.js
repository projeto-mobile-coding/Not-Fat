...
app.post("/adicionar", async (req, res) => {
  try {
    ...
    const sql =
      "INSERT INTO refeicao_usuario (usuario, refeicao, alimento) VALUES (?, ?, ?)";
...
// 7. ROTA DE MUDAR O NOME DO USUÁRIO
app.put("/usuario/:idUsuario", async (req, res) => {
  try {
    ...
    if (!novo_nome) {
      return res
        .status(400)
        .json({ erro: "Você precisa enviar o 'novo_nome'." });
    }
...
// 8. LIGAR O SERVIDOR (Sempre no final do arquivo!)
